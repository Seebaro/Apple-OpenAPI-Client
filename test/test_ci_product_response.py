# coding: utf-8

"""
    App Store Connect API

    No description provided (generated by Openapi Generator https://github.com/openapitools/openapi-generator)

    The version of the OpenAPI document: 4.0.0
    Generated by OpenAPI Generator (https://openapi-generator.tech)

    Do not edit the class manually.
"""  # noqa: E501


import unittest

from openapi_client.models.ci_product_response import CiProductResponse

class TestCiProductResponse(unittest.TestCase):
    """CiProductResponse unit test stubs"""

    def setUp(self):
        pass

    def tearDown(self):
        pass

    def make_instance(self, include_optional) -> CiProductResponse:
        """Test CiProductResponse
            include_optional is a boolean, when False only required
            params are included, when True both required and
            optional params are included """
        # uncomment below to create an instance of `CiProductResponse`
        """
        model = CiProductResponse()
        if include_optional:
            return CiProductResponse(
                data = openapi_client.models.ci_product.CiProduct(
                    type = 'ciProducts', 
                    id = '', 
                    attributes = openapi_client.models.ci_product_attributes.CiProduct_attributes(
                        name = '', 
                        created_date = datetime.datetime.strptime('2013-10-20 19:20:30.00', '%Y-%m-%d %H:%M:%S.%f'), 
                        product_type = 'APP', ), 
                    relationships = openapi_client.models.ci_product_relationships.CiProduct_relationships(
                        app = openapi_client.models.beta_app_localization_relationships_app.BetaAppLocalization_relationships_app(
                            links = openapi_client.models.relationship_links.RelationshipLinks(
                                self = '', 
                                related = '', ), 
                            data = openapi_client.models.accessibility_declaration_create_request_data_relationships_app_data.AccessibilityDeclarationCreateRequest_data_relationships_app_data(
                                type = 'apps', 
                                id = '', ), ), 
                        bundle_id = openapi_client.models.ci_product_relationships_bundle_id.CiProduct_relationships_bundleId(), 
                        workflows = openapi_client.models.analytics_report_instance_relationships_segments.AnalyticsReportInstance_relationships_segments(), 
                        primary_repositories = openapi_client.models.ci_product_relationships_primary_repositories.CiProduct_relationships_primaryRepositories(
                            meta = openapi_client.models.paging_information.PagingInformation(
                                paging = openapi_client.models.paging_information_paging.PagingInformation_paging(
                                    total = 56, 
                                    limit = 56, 
                                    next_cursor = '', ), ), ), 
                        additional_repositories = openapi_client.models.analytics_report_instance_relationships_segments.AnalyticsReportInstance_relationships_segments(), 
                        build_runs = , ), 
                    links = openapi_client.models.resource_links.ResourceLinks(
                        self = '', ), ),
                included = [
                    null
                    ],
                links = openapi_client.models.document_links.DocumentLinks(
                    self = '', )
            )
        else:
            return CiProductResponse(
                data = openapi_client.models.ci_product.CiProduct(
                    type = 'ciProducts', 
                    id = '', 
                    attributes = openapi_client.models.ci_product_attributes.CiProduct_attributes(
                        name = '', 
                        created_date = datetime.datetime.strptime('2013-10-20 19:20:30.00', '%Y-%m-%d %H:%M:%S.%f'), 
                        product_type = 'APP', ), 
                    relationships = openapi_client.models.ci_product_relationships.CiProduct_relationships(
                        app = openapi_client.models.beta_app_localization_relationships_app.BetaAppLocalization_relationships_app(
                            links = openapi_client.models.relationship_links.RelationshipLinks(
                                self = '', 
                                related = '', ), 
                            data = openapi_client.models.accessibility_declaration_create_request_data_relationships_app_data.AccessibilityDeclarationCreateRequest_data_relationships_app_data(
                                type = 'apps', 
                                id = '', ), ), 
                        bundle_id = openapi_client.models.ci_product_relationships_bundle_id.CiProduct_relationships_bundleId(), 
                        workflows = openapi_client.models.analytics_report_instance_relationships_segments.AnalyticsReportInstance_relationships_segments(), 
                        primary_repositories = openapi_client.models.ci_product_relationships_primary_repositories.CiProduct_relationships_primaryRepositories(
                            meta = openapi_client.models.paging_information.PagingInformation(
                                paging = openapi_client.models.paging_information_paging.PagingInformation_paging(
                                    total = 56, 
                                    limit = 56, 
                                    next_cursor = '', ), ), ), 
                        additional_repositories = openapi_client.models.analytics_report_instance_relationships_segments.AnalyticsReportInstance_relationships_segments(), 
                        build_runs = , ), 
                    links = openapi_client.models.resource_links.ResourceLinks(
                        self = '', ), ),
                links = openapi_client.models.document_links.DocumentLinks(
                    self = '', ),
        )
        """

    def testCiProductResponse(self):
        """Test CiProductResponse"""
        # inst_req_only = self.make_instance(include_optional=False)
        # inst_req_and_optional = self.make_instance(include_optional=True)

if __name__ == '__main__':
    unittest.main()
