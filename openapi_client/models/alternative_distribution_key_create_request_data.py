...
class AlternativeDistributionKeyCreateRequestData(BaseModel):
    """
    AlternativeDistributionKeyCreateRequestData
    """ # noqa: E501
    type: StrictStr
    attributes: AlternativeDistributionKeyCreateRequestDataAttributes
    relationships: Optional[AlternativeDistributionKeyCreateRequestDataRelationships] = None
    __properties: ClassVar[List[str]] = ["type", "attributes", "relationships"]

    @field_validator('type')
    def type_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['alternativeDistributionKeys']):
            raise ValueError("must be one of enum values ('alternativeDistributionKeys')")
        return value
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            "type": obj.get("type"),
            "attributes": AlternativeDistributionKeyCreateRequestDataAttributes.from_dict(obj["attributes"]) if obj.get("attributes") is not None else None,
            "relationships": AlternativeDistributionKeyCreateRequestDataRelationships.from_dict(obj["relationships"]) if obj.get("relationships") is not None else None
        })
        return _obj
