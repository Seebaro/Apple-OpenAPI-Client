...
class AppClipAdvancedExperienceAttributes(BaseModel):
    """
    ...
    is_powered_by: Optional[StrictBool] = Field(default=None, alias="isPoweredBy")
    place: Optional[AppClipAdvancedExperienceAttributesPlace] = None
    place_status: Optional[StrictStr] = Field(default=None, alias="placeStatus")
    business_category: Optional[StrictStr] = Field(default=None, alias="businessCategory")
    default_language: Optional[AppClipAdvancedExperienceLanguage] = Field(default=None, alias="defaultLanguage")
    __properties: ClassVar[List[str]] = ["link", "version", "status", "action", "isPoweredBy", "place", "placeStatus", "businessCategory", "defaultLanguage"]
    ...
    def status_validate_enum(cls, value):
        ...
        if value not in set(['RECEIVED', 'DEACTIVATED', 'APP_TRANSFER_IN_PROGRESS']):
            raise ValueError("must be one of enum values ('RECEIVED', 'DEACTIVATED', 'APP_TRANSFER_IN_PROGRESS')")
        return value

    @field_validator('place_status')
    def place_status_validate_enum(cls, value):
        """Validates the enum"""
        if value is None:
            return value

        if value not in set(['PENDING', 'MATCHED', 'NO_MATCH']):
            raise ValueError("must be one of enum values ('PENDING', 'MATCHED', 'NO_MATCH')")
        return value
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            ...
            "placeStatus": obj.get("placeStatus"),
