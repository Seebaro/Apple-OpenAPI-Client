...
class SubscriptionUpdateRequestData(BaseModel):
    """
    SubscriptionUpdateRequestData
    """ # noqa: E501
    type: StrictStr
    id: StrictStr
    attributes: Optional[SubscriptionUpdateRequestDataAttributes] = None
    relationships: Optional[SubscriptionUpdateRequestDataRelationships] = None
    __properties: ClassVar[List[str]] = ["type", "id", "attributes", "relationships"]

    @field_validator('type')
    def type_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['subscriptions']):
            raise ValueError("must be one of enum values ('subscriptions')")
        return value
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            "type": obj.get("type"),
            "id": obj.get("id"),
            "attributes": SubscriptionUpdateRequestDataAttributes.from_dict(obj["attributes"]) if obj.get("attributes") is not None else None,
            "relationships": SubscriptionUpdateRequestDataRelationships.from_dict(obj["relationships"]) if obj.get("relationships") is not None else None
        })
        return _obj
