...
class AccessibilityDeclarationCreateRequestData(BaseModel):
    """
    AccessibilityDeclarationCreateRequestData
    """ # noqa: E501
    type: StrictStr
    attributes: AccessibilityDeclarationCreateRequestDataAttributes
    relationships: AccessibilityDeclarationCreateRequestDataRelationships
    __properties: ClassVar[List[str]] = ["type", "attributes", "relationships"]

    @field_validator('type')
    def type_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['accessibilityDeclarations']):
            raise ValueError("must be one of enum values ('accessibilityDeclarations')")
        return value
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            "type": obj.get("type"),
            "attributes": AccessibilityDeclarationCreateRequestDataAttributes.from_dict(obj["attributes"]) if obj.get("attributes") is not None else None,
            "relationships": AccessibilityDeclarationCreateRequestDataRelationships.from_dict(obj["relationships"]) if obj.get("relationships") is not None else None
        })
        return _obj
