...
class NominationUpdateRequestData(BaseModel):
    """
    NominationUpdateRequestData
    """ # noqa: E501
    type: StrictStr
    id: StrictStr
    attributes: Optional[NominationUpdateRequestDataAttributes] = None
    relationships: Optional[NominationUpdateRequestDataRelationships] = None
    __properties: ClassVar[List[str]] = ["type", "id", "attributes", "relationships"]

    @field_validator('type')
    def type_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['nominations']):
            raise ValueError("must be one of enum values ('nominations')")
        return value
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            "type": obj.get("type"),
            "id": obj.get("id"),
            "attributes": NominationUpdateRequestDataAttributes.from_dict(obj["attributes"]) if obj.get("attributes") is not None else None,
            "relationships": NominationUpdateRequestDataRelationships.from_dict(obj["relationships"]) if obj.get("relationships") is not None else None
        })
        return _obj
