...
class WebhookPingCreateRequestData(BaseModel):
    """
    WebhookPingCreateRequestData
    """ # noqa: E501
    type: StrictStr
    relationships: WebhookPingCreateRequestDataRelationships
    __properties: ClassVar[List[str]] = ["type", "relationships"]

    @field_validator('type')
    def type_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['webhookPings']):
            raise ValueError("must be one of enum values ('webhookPings')")
        return value
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            "type": obj.get("type"),
            "relationships": WebhookPingCreateRequestDataRelationships.from_dict(obj["relationships"]) if obj.get("relationships") is not None else None
        })
        return _obj
