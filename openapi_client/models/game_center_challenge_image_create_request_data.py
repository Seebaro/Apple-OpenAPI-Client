...
class GameCenterChallengeImageCreateRequestData(BaseModel):
    """
    GameCenterChallengeImageCreateRequestData
    """ # noqa: E501
    type: StrictStr
    attributes: AppClipAdvancedExperienceImageCreateRequestDataAttributes
    relationships: Optional[GameCenterChallengeImageCreateRequestDataRelationships] = None
    __properties: ClassVar[List[str]] = ["type", "attributes", "relationships"]

    @field_validator('type')
    def type_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['gameCenterChallengeImages']):
            raise ValueError("must be one of enum values ('gameCenterChallengeImages')")
        return value
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            "type": obj.get("type"),
            "attributes": AppClipAdvancedExperienceImageCreateRequestDataAttributes.from_dict(obj["attributes"]) if obj.get("attributes") is not None else None,
            "relationships": GameCenterChallengeImageCreateRequestDataRelationships.from_dict(obj["relationships"]) if obj.get("relationships") is not None else None
        })
        return _obj
